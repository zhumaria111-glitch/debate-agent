...
| v0.1 | MVP：粘贴链接 → 结构化分析 | — | — |
| v0.2 | 导出功能 | — | — |
| v0.3 | 修复 chunk 截断 + quick_view | 单篇分析质量 | 3.52 |
| v0.3.1 | 修复空白 + 评委过滤 | 单篇分析质量 | **4.24** |
| v0.4.2 | 逐字稿清洗 + 赛制自适应（支持 哲理辩/政策辩 等多种赛制） | 端到端流水线 | 3.8 |
| v0.5 | ChromaDB + sentence-transformers RAG 知识库，跨视频对比 | 知识库 RAG | 3.4 |

> **注意：评分不可直接横向对比。** 每个版本评测的是**不同能力维度**——v0.3.1 测单篇分析准确性，v0.4.2 测多赛制流水线自动化，v0.5 测跨视频知识库检索。分数下降不代表产品变差，而是在挑战更复杂的能力。就像同一道题的不同科目，分数高低取决于科目难度。
